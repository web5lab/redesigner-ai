...
export const redesignWebsite = async ({ data }: { data: any }) => {
    try {
        ...
        if (err.response && err.response.status === 400) {
            // Throw the message from API response
...
export const updateHtmlFn = async ({ websiteId, updatedHtml }: { websiteId: string; updatedHtml: string }) => {
    try {
        const token = localStorage.getItem('authToken');
...
export const createGmailAccount = async ({ email, password, name }: { email: string; password: string; name: string }) => {
    try {
...
export const gmailLogin = async ({ email, password }: { email: string; password: string }) => {
    try {
...
export const forgetPasswordApi = async ({ email }: { email: string }) => {
    try {
...
export const resetPasswordApi = async ({ newPassword, token }: { newPassword: string; token: string }) => {
    try {
...
export const verifiyOtp = async ({ email, code, type }: { email: string; code: string; type: string }) => {
    try {
...
export const getLogs = createAsyncThunk(
    "global/getLogs",
    async ({ page = 1, limit = 10, appName = "shiva" }: { page?: number; limit?: number; appName?: string }) => {
        try {
            const token = localStorage.getItem('authToken');
            console.log("token", token);
