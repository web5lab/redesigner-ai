...
const ProtectedRoute = ({ children }) => {
  ...
  const dispatch = useDispatch();
  const websiteQueqe = useSelector((state) => state?.global?.websiteQueqe);
...
function App() {
  ...
  useEffect(() => {
    ...
    const initExtension = async () => {
      try {
        const extensionId = import.meta.env.VITE_EXTENSION_ID; // Replace with actual ID
  ...
  const sendJWTToExtension = async (token,extension) => {
