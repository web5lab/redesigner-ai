...
router.post('/users/:userId/add-balance', authenticateAdmin, checkPermission('users', 'update'), async (req, res) => {
  try {
    ...
    // Add to user balance or tickets based on type
    if (type === 'tokens') {
      const currentBalance = parseFloat(user.tokenBalances.xxxhub || '0');
      user.tokenBalances.xxxhub = (currentBalance + amount).toString();
      
      // Create transaction record
      const transaction = new Transaction({
        userId: user._id,
        walletAddress: user.walletAddress,
        transactionHash: `admin_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`,
        blockNumber: 0,
        blockHash: '0x0',
        transactionIndex: 0,
        ...
        fromAddress: req.admin._id.toString(),
      ...
      return res.status(400).json({
        success: false,
        message: 'Invalid type. Must be "tokens" or "tickets"'
      });
    }
    ...
    res.json({
      success: true,
      message: `Added ${amount} ${type} to user successfully`,
      data: {
        user: {
          id: user._id,
          walletAddress: user.walletAddress,
...
router.post('/users/:userId/deduct-balance', authenticateAdmin, checkPermission('users', 'update'), async (req, res) => {
  try {
    const { userId } = req.params;
    const { amount, type } = req.body;
    ...
    // Deduct from user balance or tickets based on type
    if (type === 'tokens') {
      const currentBalance = parseFloat(user.tokenBalances.xxxhub || '0');
      ...
      const transaction = new Transaction({
        ...
        transactionIndex: 0,
        type: 'admin_adjustment',
        purpose: 'Admin balance deduction',
        amount: (-amount).toString(),
        tokenType: 'XXX',
        tokenAddress: '0x0',
        usdValue: 0,
        ...
        toAddress: req.admin._id.toString()
      });
      
      await transaction.save();
    } else if (type === 'tickets') {
      if (user.currentTickets < amount) {
        return res.status(400).json({
          success: false,
    ...
    res.json({
      success: true,
      message: `Deducted ${amount} ${type} from user successfully`,
      data: {
        user: {
          id: user._id,
          walletAddress: user.walletAddress,
